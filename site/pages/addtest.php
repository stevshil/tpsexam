...
error_reporting(E_ALL);
	...
	function qList($CID=null) {
		if ( isset($CID) ) {
			if ( $CID == '' ) {
				$CID="select questionCategory.CategoryID from  questionCategory";
			}
			$questionsData = dbquery("questions.QID,Question,AID,Correct,AnswerText,CategoryID,CategoryName","questions,answers,questionCategory,categories","WHERE questions.QID=answers.QID and questions.QID=questionCategory.QID and questionCategory.CategoryID=categories.CatID AND questionCategory.CategoryID in (" . $CID . ") GROUP BY QID");
		} else { 
...
<table width='80%'>
<tr><td align=center colspan=2>In both columns use the CTRL key to select multiple<br>&nbsp;</td></tr>
<tr><td>Select Questions</td><td>Filter by Category</td></tr>
<tr><td><select name='QID' onChange='return checkSelect();' size=15 multiple>
...
<select name='CID' onChange='filterCat(this.options)' size=15 multiple>
<option value=''>--- Please select question ---</option>
<?php
	$cList=catList();
